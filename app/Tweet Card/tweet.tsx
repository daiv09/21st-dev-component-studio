...
const Icons = {
  ...
  Retweet: (props: React.SVGProps<SVGSVGElement>) => (
    <svg viewBox="0 0 24 24" aria-hidden="true" fill="currentColor" {...props}>
  ...
  Like: (props: React.SVGProps<SVGSVGElement> & { filled?: boolean }) => (
    <svg viewBox="0 0 24 24" aria-hidden="true" fill={props.filled ? "currentColor" : "none"} stroke="currentColor" strokeWidth={props.filled ? "0" : "2"} {...props}>
      <g>
        {props.filled ? (
          ...
          <path strokeLinecap="round" strokeLinejoin="round" d="M16.697 5.5c-1.222-.06-2.679.51-3.89 2.16l-.805 1.09-.806-1.09C9.984 6.01 8.526 5.44 7.304 5.5c-1.243.07-2.349.78-2.91 1.91-.928 1.88-.872 4.01.46 6.47 1.256 2.3 3.746 4.7 7.946 7.11 4.199-2.41 6.69-4.81 7.946-7.11 1.332-2.46 1.388-4.59.46-6.47-.56-1.13-1.667-1.84-2.908-1.91zM2.529 6.55c.986-1.98 2.89-3.3 5.088-3.41 2.046-.11 3.992.83 5.432 2.45 1.44-1.62 3.385-2.56 5.431-2.45 2.199.11 4.103 1.43 5.089 3.41 1.526 3.07 1.289 6.29-.462 9.5-1.66 3.04-4.66 5.92-9.456 8.58l-1.002.56-1.002-.56c-4.796-2.66-7.796-5.54-9.457-8.58-1.75-3.21-1.987-6.43-.46-9.5z"></path>
        )}
      </g>
    </svg>
  ),
...
// --- Component ---
export function TweetCard({ author, content, timestamp, stats, mediaUrl, tags, className }: TweetProps) {
  ...
  return (
    <motion.article
      ...
      className={cn(
        ...
        // Dark Mode Styles
        "dark:bg-black/60 dark:border-white/10 dark:shadow-none",
        className
      )}
    >
      ...
      <div className="relative flex gap-4">
        ...
        <div className="flex-shrink-0">
          <div className="relative h-12 w-12 overflow-hidden rounded-full ring-2 ring-transparent transition-all group-hover:ring-sky-500/50">
            <Image 
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex flex-wrap items-center gap-1.5 text-[15px] leading-5">
              <span className="font-bold text-gray-900 dark:text-white truncate transition-colors group-hover:text-sky-500 dark:group-hover:text-sky-400">
                {author.name}
              </span>
              {author.isVerified && (
                <span className="text-sky-500" title="Verified">
                  <Icons.Verified className="h-[18px] w-[18px]" />
                </span>
              )}
              ...
              <span className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:underline cursor-pointer transition-colors">
                {timestamp}
              </span>
            </div>
          ...
          <div className="mt-2 whitespace-pre-wrap text-[15px] leading-relaxed text-gray-700 dark:text-gray-200">
            ...
            {tags && tags.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2 text-sky-600 dark:text-sky-400">
                    {tags.map(tag => <span key={tag} className="hover:underline cursor-pointer">#{tag}</span>)}
                </div>
            )}
          </div>
          ...
          {mediaUrl && (
            ...
            >
              <div className="absolute inset-0 bg-linear-to-t from-black/5 dark:from-black/20 to-transparent z-10" />
              <img src={mediaUrl} alt="Tweet media" className="h-auto w-full object-cover transition-transform duration-700 group-hover:scale-[1.02]" />
            </motion.div>
          )}
          ...
          <div className="mt-4 flex justify-between gap-2 text-gray-500 dark:text-gray-400 pr-4">
            <ActionButton 
              icon={<Icons.Reply className="h-[18px] w-[18px]" />} 
              count={stats.replies} 
              colorClass="hover:text-sky-500 group-hover/btn:bg-sky-500/10" 
            />
            <ActionButton 
              icon={<Icons.Retweet className="h-[18px] w-[18px]" />} 
              count={retweetCount} 
              active={isRetweeted}
              colorClass="hover:text-green-500 group-hover/btn:bg-green-500/10" 
            ...
            <ActionButton 
              icon={
                ...
                >
                  <Icons.Like filled={isLiked} className="h-[18px] w-[18px]" />
                </motion.div>
              } 
              count={likeCount} 
              active={isLiked}
              colorClass="hover:text-pink-500 group-hover/btn:bg-pink-500/10" 
              activeClass="text-pink-500"
              onClick={handleLike}
            />
             <ActionButton 
              icon={<Icons.Chart className="h-[18px] w-[18px]" />} 
              count={0} 
              label={stats.views} 
              colorClass="hover:text-sky-500 group-hover/btn:bg-sky-500/10" 
            />
            <div className="group/btn flex items-center transition-colors hover:text-sky-500 cursor-pointer">
              <div className="flex h-8 w-8 items-center justify-center rounded-full transition-colors group-hover/btn:bg-sky-500/10">
                 <Icons.Share className="h-[18px] w-[18px]" />
              </div>
            </div>
          </div>
        </div>
...
function ActionButton({ icon, count, label, active, colorClass, activeClass, onClick }: ActionButtonProps) {
  return (
    <button 
      onClick={onClick}
      className={cn(
        "group/btn flex items-center gap-1.5 transition-colors text-[13px] hover:scale-105 active:scale-95",
        colorClass,
        active && activeClass
      )}
    >
      ...
      <span className={cn("tabular-nums font-medium", active && activeClass)}>
        {label ? label : (count > 0 ? formatCount(count) : '')}
      </span>
    </button>
  );
}
...
export default function TweetCardDemo() {
  const demoData: TweetProps = {
    author: {
      name: "Dev Projects",
      handle: "elena_design",
      avatarSrc: "https://images.unsplash.com/photo-1511367461989-f85a21fda167?q=80&w=1331&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      isVerified: true,
    },
    content: "Just launched a new tweet component on 21st.dev today, built with React, Framer Motion, and Tailwind, complete with interactive stats, hover lighting, and support for rich media previews.",
  tags: ["React", "FramerMotion", "TailwindCSS", "21st.dev"],
    timestamp: "18m",
    mediaUrl: "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8c2t5c2NyYXBlcnxlbnwwfHwwfHx8MA%3D%3D",
    stats: {
      replies: 142,
      retweets: 856,
      likes: 4200,
  ...
  return (
    // Updated container: neutral gray for light mode, deep black for dark mode
    <div className="flex min-h-screen w-full flex-col items-center justify-center gap-8 bg-gray-100 dark:bg-[#050505] p-8 relative transition-colors duration-500">
      
      {/* Background Ambience (Visible mostly in Dark Mode) */}
      <div className="fixed inset-0 pointer-events-none opacity-0 dark:opacity-100 transition-opacity duration-500">
         ...
         <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-sky-900/20 blur-[120px] rounded-full" />
      </div>

      <div className="z-10 flex flex-col items-center gap-4">
          <h2 className="text-lg font-medium text-gray-500 dark:text-white/40 tracking-widest uppercase">Social Card Component</h2>
          <TweetCard {...demoData} />
      </div>
    </div>
  );
