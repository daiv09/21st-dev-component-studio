...
import { SundialGnomon } from "./sundial-gnomon";
  ...
  return (
    <main className="min-h-screen w-full bg-neutral-950 text-neutral-100 flex flex-col lg:flex-row items-center justify-center gap-10 p-8 font-mono">
      <div className="flex flex-col items-center gap-4">
        <div className="text-[10px] tracking-[0.35em] uppercase text-neutral-500">Instrument · 11</div>
        <h1 className="text-2xl tracking-tight">Sundial Gnomon</h1>
        <SundialGnomon latitude={latitude} hourAngle={hourAngle} declination={declination} size={480} />
      </div>
      <aside className="w-full max-w-xs border border-neutral-800 bg-neutral-900 p-5 shadow-[6px_6px_0_#000] flex flex-col gap-4">
        ...
        <label className="flex flex-col gap-1 text-[10px] uppercase tracking-widest text-neutral-400">
          <span className="flex justify-between"><span>Hour angle°</span><span>{hourAngle}</span></span>
          <input type="range" min={-90} max={90} step={1} value={hourAngle} onChange={(e) => setHourAngle(Number(e.target.value))} className="accent-neutral-200" />
        </label>
        <label className="flex flex-col gap-1 text-[10px] uppercase tracking-widest text-neutral-400">
          <span className="flex justify-between"><span>Declination°</span><span>{declination}</span></span>
          <input type="range" min={-23.5} max={23.5} step={0.5} value={declination} onChange={(e) => setDeclination(Number(e.target.value))} className="accent-neutral-200" />
        </label>
      </aside>
    </main>
  );
