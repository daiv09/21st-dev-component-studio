// Built using Hyperiux Vault (Linear-to-Orbit Edition)
"use client";
...
const IMG = {
  plane: `${IMG_BASE}/img8.png`,
  painting: `${IMG_BASE}/img2.png`,
  breaker: `${IMG_BASE}/img7.png`,
  dog: `${IMG_BASE}/img3.png`,
  footballer: `${IMG_BASE}/img5.png`,
  jacket: `${IMG_BASE}/img6.png`,
  meadow: `${IMG_BASE}/img1.png`,
  stripes: `${IMG_BASE}/img4.png`,
} as const;
...
const PARALLAX_INTENSITY = 2.8;
const SPRING_CONFIG = { stiffness: 80, damping: 22, mass: 0.7 };
const PROGRESS_SPRING = { stiffness: 100, damping: 30, restDelta: 0.0001 };

const SUB = "From linear alignment to dynamic spatial architecture.";
...
function Card({
  ...
  const linearRotate = flat ? 0 : card.linearRotate ?? 0;
  const linearOffset = card.linearOffset ?? { x: 0, y: 0 };
  ...
  const translate = useTransform(
    ...
    ([p, px, py]: number[]) => {
      ...
      const tx = linearOffset.x + (endX - linearOffset.x) * easeP;
      const ty = linearOffset.y + (endY - linearOffset.y) * easeP;
  ...
  const rotate = useTransform(progress, [0, 1], [linearRotate, endRotate]);
  ...
  return (
    ...
    >
      <div
        className="relative h-full w-full overflow-hidden shadow-2xl shadow-black/20 dark:shadow-black/60 ring-1 ring-black/10 dark:ring-white/15 transition-shadow duration-300 hover:shadow-black/40 max-md:rounded-[4vw]"
        style={{ borderRadius: `${cardRadius}px` }}
      >
        <div className="absolute inset-0 bg-gradient-to-tr from-black/25 via-transparent to-white/10 opacity-70 pointer-events-none z-10" />
        <img
          src={item.src}
          alt={item.alt ?? ""}
          draggable={false}
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-30 blur-[100px]">
          <div className="w-[50vw] h-[50vw] rounded-full bg-neutral-300 dark:bg-neutral-800 mix-blend-multiply dark:mix-blend-screen" />
        </div>
        ...
        >
          ...
          >
            Linear <span className="font-normal opacity-50">To</span> Spatial.
          </h2>
          <p
        ...
        {showScrollHint && (
          ...
          >
            <span className="opacity-75">Scroll to Expand</span>
            <div className="w-[1px] h-6 bg-current opacity-25 relative overflow-hidden">
              <motion.div 
